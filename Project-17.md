...
```
### STEP 6: Creating Certificate From Amazon Certificate Manager
...
```
### STEP 7: Creating Application Load Balancer and Target Groups
#### Creating a file called alb.tf and entering the following codes:
```
# ----------------------------
...
resource "aws_lb_listener_rule" "tooling-listener" {
  ...
  condition {
    host_header {
      values = ["tooling.updevops.tk"]
    }
  }
}
```
### STEP 8: Creating An Auto Scaling Group
...
resource "aws_sns_topic" "fola-sns" {
...
resource "aws_autoscaling_notification" "fola_notifications" {
  ...
  topic_arn = aws_sns_topic.fola-sns.arn
}
...
```
#### Creating a new file called asg-webserver.tf and entering the following codes which creates launch templates and Auto Scaling Group for both the wordpress and tooling webserver:
```
...
```
### STEP 9: Creating four files that will be used as user data for launching the four different servers:
